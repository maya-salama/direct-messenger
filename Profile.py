...
# THIS CODE RIGHT NOW, though can you certainly take a look at it if you
# are curious since we already covered a bit of the JSON format in class.
#
import json
import time
...
code. It is raised when attempting to load or save Profile objects to file the
system.
...
code. It is raised when attempting to deserialize a dsu file to a Profile
object.
...
class Post(dict):
    ...
    The property method is used to support get and set capability for
    entry and time values. When the value for entry is changed, or set,
...
class Profile:
    ...
    by each new user created within your program for a2. Pay close attention
    to the properties and functions in this class as you will need to make use
    ...
    So if multiple Posts objects are created, but added to the Profile
    in a different order, it is possible for the list to not be sorted
    by the Post.timestamp property. So take caution as to how you
